...
La carpeta `entrega/` trae el live script ejecutado, el CSV al lado y el PDF de la exposición, y está comprimida en `Exposicion_Bank_codigo.zip`. El dataset va en `data/bank-full.csv`, se saca de `bank+marketing.zip` de la Primera actividad. La exposición está en `exposicion_bank_final.pptx` y su PDF en `exposicion_bank_final.pdf`.
